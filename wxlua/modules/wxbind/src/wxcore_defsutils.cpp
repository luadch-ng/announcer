...
static int LUACALL wxLua_wxProcess_Kill(lua_State *L)
{
    ...
    int flags = (argCount >= 3 ? (int)wxlua_getnumbertype(L, 3) : (int)wxKILL_NOCHILDREN);
    // wxSignal sig = wxSIGTERM
    wxSignal sig = (argCount >= 2 ? (wxSignal)wxlua_getenumtype(L, 2) : (wxSignal)wxSIGTERM);
    // int pid
    int pid = (int)wxlua_getnumbertype(L, 1);
    // call Kill
...
static int LUACALL wxLua_wxProcess_constructor(lua_State *L)
{
    ...
    int argCount = lua_gettop(L);
    // int nId = wxID_ANY
    int nId = (argCount >= 2 ? (int)wxlua_getnumbertype(L, 2) : (int)wxID_ANY);
    // wxEvtHandler parent = NULL
    wxEvtHandler * parent = (argCount >= 1 ? (wxEvtHandler *)wxluaT_getuserdatatype(L, 1, wxluatype_wxEvtHandler) : (wxEvtHandler *)NULL);
    // call constructor
...
static int LUACALL wxLua_wxKeyboardState_constructor(lua_State *L)
{
    ...
    int argCount = lua_gettop(L);
    // bool metaDown = false
    bool metaDown = (argCount >= 4 ? wxlua_getbooleantype(L, 4) : (bool)false);
    // bool altDown = false
    bool altDown = (argCount >= 3 ? wxlua_getbooleantype(L, 3) : (bool)false);
    // bool shiftDown = false
    bool shiftDown = (argCount >= 2 ? wxlua_getbooleantype(L, 2) : (bool)false);
    // bool controlDown = false
    bool controlDown = (argCount >= 1 ? wxlua_getbooleantype(L, 1) : (bool)false);
    // call constructor
...
static int LUACALL wxLua_wxTimer_SetOwner(lua_State *L)
{
    ...
    int argCount = lua_gettop(L);
    // int id = -1
    int id = (argCount >= 3 ? (int)wxlua_getnumbertype(L, 3) : (int)-1);
    // wxEvtHandler owner
...
static int LUACALL wxLua_wxTimer_Start(lua_State *L)
{
    ...
    int argCount = lua_gettop(L);
    // bool oneShot = false
    bool oneShot = (argCount >= 3 ? wxlua_getbooleantype(L, 3) : (bool)false);
    // int milliseconds = -1
    int milliseconds = (argCount >= 2 ? (int)wxlua_getnumbertype(L, 2) : (int)-1);
    // get this
    wxTimer * self = (wxTimer *)wxluaT_getuserdatatype(L, 1, wxluatype_wxTimer);
    // call Start
...
static int LUACALL wxLua_wxTimer_constructor1(lua_State *L)
{
    ...
    int argCount = lua_gettop(L);
    // int id = -1
    int id = (argCount >= 2 ? (int)wxlua_getnumbertype(L, 2) : (int)-1);
    // wxEvtHandler owner
