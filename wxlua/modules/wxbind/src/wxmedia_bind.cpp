...
static int LUACALL wxLua_wxMediaCtrl_Create(lua_State *L)
{
    ...
    const wxString name = (argCount >= 10 ? wxlua_getwxStringtype(L, 10) : (const wxString)wxString(wxT("wxMediaCtrl")));
    // const wxValidator val = wxDefaultValidator
    const wxValidator * val = (argCount >= 9 ? (const wxValidator *)wxluaT_getuserdatatype(L, 9, wxluatype_wxValidator) : (const wxValidator *)&wxDefaultValidator);
    // const wxString szBackend = ""
    const wxString szBackend = (argCount >= 8 ? wxlua_getwxStringtype(L, 8) : (const wxString)wxString(wxEmptyString));
    // long style = 0
    ...
    const wxString fileName = (argCount >= 4 ? wxlua_getwxStringtype(L, 4) : (const wxString)wxString(wxEmptyString));
    // wxWindowID winid
...
static int LUACALL wxLua_wxMediaCtrl_ShowPlayerControls(lua_State *L)
{
    ...
    wxMediaCtrlPlayerControls flags = (argCount >= 2 ? (wxMediaCtrlPlayerControls)wxlua_getenumtype(L, 2) : (wxMediaCtrlPlayerControls)wxMEDIACTRLPLAYERCONTROLS_DEFAULT);
    // get this
    wxMediaCtrl * self = (wxMediaCtrl *)wxluaT_getuserdatatype(L, 1, wxluatype_wxMediaCtrl);
...
static int LUACALL wxLua_wxMediaCtrl_constructor1(lua_State *L)
{
    ...
    int argCount = lua_gettop(L);
    // const wxString name = "wxMediaCtrl"
    const wxString name = (argCount >= 9 ? wxlua_getwxStringtype(L, 9) : (const wxString)wxString(wxT("wxMediaCtrl")));
    // const wxValidator val = wxDefaultValidator
    const wxValidator * val = (argCount >= 8 ? (const wxValidator *)wxluaT_getuserdatatype(L, 8, wxluatype_wxValidator) : (const wxValidator *)&wxDefaultValidator);
    // const wxString szBackend = ""
    const wxString szBackend = (argCount >= 7 ? wxlua_getwxStringtype(L, 7) : (const wxString)wxString(wxEmptyString));
    // long style = 0
    ...
    // const wxString fileName = ""
    const wxString fileName = (argCount >= 3 ? wxlua_getwxStringtype(L, 3) : (const wxString)wxString(wxEmptyString));
    // wxWindowID winid
