...
modules as the CLI, plus a wxLua-based UI. Six tabs (actual labels
from `notebook:AddPage` calls):

### Tab 1 - "Hub Account"

Form-bound to `cfg/hub.lua`: addr / port / nick / pass / keyprint,
plus the friendly `name` field for display purposes. Save writes
back to the file. The bot identity (`cfg/id.lua`) is not editable
from the GUI - it's auto-generated, see section 1.

### Tab 2 - "Announcer Config"

Form-bound to `cfg/cfg.lua`: announce interval, sleep / socket
timeouts, log-rotation size, slots / share / upload speed claims,
tray-icon toggle, plus the bot description (`botdesc`).

### Tab 3 - "Announcer Rules"

Read-only view of the configured watched rules (from `cfg/rules.lua`).
The list shows rulename, category, path, command, and filter
flags at a glance. To add / remove / edit rules, use Tab 4.

### Tab 4 - "Add/Remove Rules"

Add new rules via a modal dialog, remove existing ones, or clone
an existing rule as a template. The add dialog enforces the
safe-name char set (letters, digits, dot, underscore, dash, parens) -
invalid characters show a status-bar hint and keep OK disabled (PR #40).

Note: `cfg/sslparams.lua` (TLS cert paths, protocol, ciphers) has
no GUI form - edit the file directly. Defaults are TLSv1.3 with
modern AEAD suites; the cert itself is generated by
`certs/make_cert.{sh,bat}`.

### Tab 5 - "Add/Remove Categories"

Same shape as Tab 4 for category names (`cfg/categories.lua`).
Categories must exist before they can be selected in a rule.
...
loaded (only re-reads when the file's size or mtime changed; the
status-line spam fix from PR #35). Files larger than 10 MB are
refused to prevent the wxTextCtrl from crashing - see section 6
for what to do then.

### CONNECT / DISCONNECT buttons

The connect/disconnect controls are on a separate panel below the
notebook, not inside any specific tab. CONNECT spawns
`frontends/gui/spawned_worker.lua` as a child process that runs the
actual `net.loop()`. Status updates flow back via `core/status.lua`
(file-IPC, atomic write-then-rename to avoid mid-write races - see
the Phase 3 Tier 3 fix). DISCONNECT signals the child to terminate.
